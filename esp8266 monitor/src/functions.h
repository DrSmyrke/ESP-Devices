...
uint8_t rtsPacket[16] = {
      /*  0 - 1  */ 0xB4, 0x00, //type
      /*  2 - 3  */ 0xB2, 0x00, //duration (SDK takes care of that)
      /*  4 - 9  */ 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB,//reciever (target)
      /* 10 - 15  */ 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB//transmitter (source)
    };
...
void buildDeauth(const uint8_t* apMAC, const uint8_t* cliMAC, uint8_t type){
  packetSize = 0;
...
void buildRTS(const uint8_t* targetMAC, const uint8_t* sourceMAC){
  packetSize = 0;
  for(int i=0;i<sizeof(rtsPacket);i++){
    packet[i] = rtsPacket[i];
    packetSize++;
  }

  for(int i=0;i<6;i++){
    //set target (client)
    packet[4+i] = targetMAC[i];
    //set source (AP)
    packet[10+i] = packet[16+i] = sourceMAC[i];
  }
}
...
void printPkt(uint8_t *buf, uint16_t len, bool format = false)
{
	if( !CheckBit( buf[12], 2 ) && !CheckBit( buf[12], 3 ) ){
		Serial.print( "MF" );Serial.print(" ");
	}
	if( CheckBit( buf[12], 2 ) && !CheckBit( buf[12], 3 ) ){
		Serial.print( "CF" );Serial.print(" ");
	}
	if( !CheckBit( buf[12], 2 ) && CheckBit( buf[12], 3 ) ){
